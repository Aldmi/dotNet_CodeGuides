...
public class CarSmallRepository
{
    private readonly IMongoCollection<CarSmall> _cars;
    private readonly ConnectionThrottlingPipeline _connectionThrottlingPipeline;
    public const string CollectionName = "Cars";

    public CarSmallRepository(IMongoCollection<CarSmall> cars, ConnectionThrottlingPipeline connectionThrottlingPipeline)
    {
        _cars = cars;
    ...
    public async Task<List<CarSmall>> Get(Expression<Func<CarSmall, bool>> predicate)
    {
        return await _cars.Find(predicate).ToListAsync();
    }
    
    
    public async Task<List<CarSmall>> Get(
        Expression<Func<CarSmall, bool>> predicate,
        Expression<Func<CarSmall, object>>? sortBy = null,
        int? limit = null)
    {
    ...
    public async Task<CarSmall?> Get(Guid carId)
    {
       return await _cars.Find(c => c.Id == carId).FirstOrDefaultAsync();
    }
    
    
    public async Task<Guid?> AddOrReplace(CarSmall car)
    {
    ...
    public async Task<long> Delete(Expression<Func<CarSmall, bool>> predicate)
    {
